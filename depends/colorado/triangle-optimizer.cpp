...
void Forsyth::OptimizeFaces (const uint * indexList, uint indexCount, uint vertexCount, 
							 uint * newIndexList, uint lruCacheSize) {
	...
	for (uint i = 0; i < indexCount; i += 3) {
		...
		for (uint v = 0; v < 3; ++v) {
			...
			std::vector <uint>::iterator begin = activeFaceList.begin ();
			begin += vertexData.activeFaceListStart;
			std::vector <uint>::iterator end = begin;
			end += vertexData.activeFaceListSize;
			std::vector <uint>::iterator it = std::find (begin, end, bestFace);
			assert (it != end);
			std::swap (*it, *(end - 1));
