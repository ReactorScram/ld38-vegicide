...
--]]------------------------------------------------------------------------
function M.getparam (t_in, options, params)
  local t_out = {}
...
function M.getParamsAndAssert (t_in, options, params)
	local out = M.getparam (t_in, lume.keys (options), params)
	assert (out)
	
	local function checkParamType (name, typeName)
		return out [name] == nil or type (out [name] == typeName)
	end
	
	local function assertParamType (...)
		assert (checkParamType (...))
	end
